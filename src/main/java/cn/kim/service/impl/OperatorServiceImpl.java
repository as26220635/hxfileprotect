...
public class OperatorServiceImpl extends BaseServiceImpl implements OperatorService {
    ...
    @Override
    @Transactional
    public Map<String, Object> headPortrait(MultipartFile fileUpload, Map<String, Object> mapParam) {
        ...
        try {
            ...
            //处理头像
            Map<String, Object> configure = Maps.newHashMapWithExpectedSize(10);
            configure.put("SF_TABLE_ID", operatorId);
            configure.put("SF_TABLE_NAME", TableName.SYS_ACCOUNT_INFO);
            configure.put("SF_TYPE_CODE", TableName.SYS_ACCOUNT_INFO);
            configure.put("SF_SEE_TYPE", STATUS_SUCCESS);

            //压缩头像
            configure.put("isCut", true);
            configure.put("zoomX", mapParam.get("x"));
            configure.put("zoomY", mapParam.get("y"));
            configure.put("zoomW", mapParam.get("w"));
            configure.put("zoomH", mapParam.get("h"));
            configure.put("scaleWidth", toString(mapParam.get("sw")).replace("px",""));
            configure.put("scaleHeight", toString(mapParam.get("sh")).replace("px",""));
            Map<String, Object> result = FileUtil.saveImgFile(fileUpload, configure);
            ...
            paramMap.put("SAI_AVATAR",result.get("id"));
            baseDao.update(NameSpace.OperatorMapper, "updateAccountInfo", paramMap);

            activeUser.setAvatar(toString(idEncrypt(result.get("id"))));
            AuthcUtil.setCurrentUser(activeUser);

            resultMap.put(MagicValue.LOG, "修改头像");
