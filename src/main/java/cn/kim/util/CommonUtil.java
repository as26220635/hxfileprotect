...
//            throw new InvalidKeyException("无效的KEY!");
    ...
    public static String getUrlParamsJoin(String smUrl, String smUrlParams) {
        StringBuilder builder = new StringBuilder();
        builder.append(smUrl.contains("?") ? smUrl + "&" : smUrl + "?");
        //去除重复参数
        for (String param : smUrlParams.split("&")) {
            if (!ValidateUtil.isEmpty(param)) {
                if (!smUrl.contains(param)) {
                    builder.append("&" + param);
                }
            }

        }
        return javaScriptStringEnc(builder.toString());
    }
        ...
        if (TextUtil.toString(smUrl).contains(MagicValue.DATA_GRID)) {
            ...
            if (!ValidateUtil.isEmpty(smUrl)) {
                try {
                    ...
                    if (!smUrl.contains("SM_ID")) {
                        smUrl = getUrlParamsJoin(smUrl, "&SM_ID=" + id + "&");
                    }
                } catch (InvalidKeyException e) {
                }
            }
