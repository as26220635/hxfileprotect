...
<%--                    </li>--%>
                    ...
                    <li class="dropdown user user-menu">
                        <a href="javascript:void(0)" class="dropdown-toggle" data-toggle="dropdown">
                            <c:choose>
                                <c:when test="${not empty activeUser.avatar}">
                                    <img data-avator="true" src="${BASE_URL}${Url.IMG_URL}${activeUser.avatar}" class="user-image"
                                         alt="User Image">
                                </c:when>
                                <c:otherwise>
                                    <img data-avator="true" src="${BASE_URL}resources/static/dist/img/avatar_default.jpg" class="user-image"
                                         alt="User Image">
                                </c:otherwise>
                            </c:choose>
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li class="user-header">
                                <c:choose>
                                    <c:when test="${not empty activeUser.avatar}">
                                        <img data-avator="true" src="${BASE_URL}${Url.IMG_URL}${activeUser.avatar}" class="img-circle"
                                             alt="User Image">
                                    </c:when>
                                    <c:otherwise>
                                        <img data-avator="true" src="${BASE_URL}resources/static/dist/img/avatar_default.jpg" class="img-circle"
                                             alt="User Image">
                                    </c:otherwise>
                                </c:choose>
                                <p>
...
<script>
    ...
    //修改头像
    $('#editHeadPortraitBtn').on('click', function () {
        ajax.getHtml('${BASE_URL}${Url.EDIT_HEADPORTRAIT}', {}, function (html) {
            model.show({
                title: '修改头像',
                content: html,
                footerModel: model.footerModel.ADMIN,
                isConfirm: true,
                confirm: function (model) {
                    ajax.file('${BASE_URL}${Url.EDIT_HEADPORTRAIT}', $('#uploadImg'), function (data) {
                        if (data.code == STATUS_SUCCESS) {
                            $('img[data-avator]').attr('src', '${BASE_URL}${Url.IMG_URL}' + data.id);
                        }
                        ajaxReturn.data(data, model, null, null);
                    });
                }
